...
def exp_upper_poly(c,idx,n,deg=8):
    # even-degree alternating Taylor upper bound for exp(-c x), valid for c*x in [0,1]
    out={}
    for j in range(deg+1):
        coef=Fraction((-1)**j, factorial(j)) * (Fraction(c)**j)
        if coef:
            exp=[0]*n; exp[idx]=j; out[tuple(exp)]=coef
    return clean(out)

def group_factors(kind,param,idx,n,q=None):
    if kind=='UG':
    ...
    if kind=='GS':
        if param=='inf':
            # For layer-a envelope, the linear chord is the intended safe upper.
            # For layer-1 GS∞, the chord is too loose; use a degree-8 Taylor upper.
            if q == a_layer:
                D=add(const(1,n), smul(-chord_slope, var(idx,n)))
            else:
                D=exp_upper_poly(lam,idx,n,8)
...
def slack_poly(blocks):
    ...
    for i,(kind,param,q) in enumerate(blocks):
        D,E,A,B=group_factors(kind,param,i,n,q); Ds.append(D); Es.append(E); al.append(A); be.append(B); qs.append(q)
    X={}
...
def check_poly(poly,n,max_depth=12,deg_raise=0):
...
def check_case(case,max_depth=12,deg_raise=0):
    idx,name,blocks=case; poly,n=slack_poly(blocks)
    # Fast path: if the global simplex Bernstein certificate is nonnegative,
    # it is certainly valid on the nontrivial supported region sum_x >= c.
    # This avoids expensive affine substitution/triangulation for many cases.
    global_deg=degree(poly)+deg_raise
    gmn,gneg,gidx,gdeg=bernstein_min_standard(poly,n,global_deg)
    if gneg==0:
        return {'idx':idx,'name':name,'blocks':str(blocks),'ok':True,'min':str(gmn),'nodes':1,'fails':[],'nfails':0,'degree':gdeg,'degree_raise':deg_raise,'M_upper':'layer-a chord; layer-1 GSinf degree-8 Taylor upper','region':'global simplex (implies restricted region)'}
    ok,mn,nodes,fails,deg=check_poly(poly,n,max_depth,deg_raise)
    return {'idx':idx,'name':name,'blocks':str(blocks),'ok':ok,'min':str(mn),'nodes':nodes,'fails':fails,'nfails':len(fails),'degree':deg,'degree_raise':deg_raise,'M_upper':'layer-a chord; layer-1 GSinf degree-8 Taylor upper','region':'sum_x >= '+str(c_supported),'global_min':str(gmn),'global_neg':gneg}
...
def save(path,rows):
    ...
    data={'rho':str(rho),'M_upper':'layer-a chord; layer-1 GSinf degree-8 Taylor upper','automatic_region':'e >= 1/rho = '+str(Fraction(1,1)/rho),'checked_region':'sum_x >= '+str(c_supported),'total_cases':len(cases()),'rows':arr,'count':len(arr),'bad':bad,'bad_count':len(bad)}
    tmp=path+'.tmp'; json.dump(data,open(tmp,'w'),indent=2); os.replace(tmp,path)

def main():
    import argparse
    ap=argparse.ArgumentParser(); ap.add_argument('--start',type=int,default=0); ap.add_argument('--end',type=int,default=180); ap.add_argument('--out',default='regenerated_180_cases.json'); ap.add_argument('--max-depth',type=int,default=12); ap.add_argument('--degree-raise',type=int,default=0); ap.add_argument('--limit',type=int)
